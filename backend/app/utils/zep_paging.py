...
This module wraps the auto-pagination logic (including per-page retries)
and returns a complete list transparently to the caller.
"""
...
def _fetch_page_with_retry(
    ...
    **kwargs: Any,
    ...
    """Fetch a single page, retrying with exponential backoff on failure. Only retries transient network/IO errors."""
    if max_retries < 1:
...
def fetch_all_nodes(
    ...
    """Fetch all graph nodes with pagination, returning at most max_items entries (default 2000). Each page request includes retries."""
    all_nodes: list[Any] = []
