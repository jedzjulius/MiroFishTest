...
def get_graph_entities(graph_id: str):
    ...
    Returns only nodes that match predefined entity types (nodes whose Labels are not only Entity)
...
def _check_simulation_prepared(simulation_id: str) -> tuple:
    ...
    # List of required files (excluding scripts; scripts live in backend/scripts/)
    required_files = [
        "state.json",
    ...
    try:
        ...
        # If config_generated=True and files exist, preparation is considered complete
        ...
        if status in prepared_statuses and config_generated:
            ...
            # If status is "preparing" but files are ready, automatically update status to "ready"
            if status == "preparing":
                try:
...
def prepare_simulation():
    """
    Prepare the simulation environment (async task, all parameters are intelligently generated by LLM)
    ...
    3. Generate OASIS Agent Profiles for each entity (with retry logic)
    4. Intelligently generate simulation configuration via LLM (with retry logic)
    ...
    Returns:
        {
            ...
            "data": {
                ...
                "message": "Prepare task started|Preparation already complete",
    ...
    try:
        ...
        # Check whether preparation has already been completed (to avoid redundant generation)
        if not force_regenerate:
            ...
            if is_prepared:
                ...
                logger.info(f"Simulation {simulation_id} is not yet prepared; a preparation task will be started")
        ...
        # ========== Synchronously fetch entity count (before starting background task) ==========
        ...
        try:
            ...
            # Save entity count to state (so the frontend can fetch it immediately)
            ...
            # Failure does not affect subsequent flow; the background task will re-fetch
...
def get_prepare_status():
    ...
    2. Check whether preparation has already been completed via simulation_id
    ...
    try:
        ...
        if not task:
            # Task not found; if simulation_id is available, check whether preparation is complete
            if simulation_id:
                is_prepared, prepare_info = _check_simulation_prepared(simulation_id)
                if is_prepared:
                    return jsonify({
...
def _get_report_id_for_simulation(simulation_id: str) -> str:
    """
    Get the latest report_id associated with the given simulation
...
def get_simulation_history():
    ...
    Used for the home page history display; returns a rich simulation list with project name, description, etc.
            ...
            "data": [
                {
                    ...
                    "simulation_requirement": "If the organization releases...",
                    "status": "completed",
                    "entities_count": 68,
                    "profiles_count": 68,
    ...
    try:
        ...
        for sim in simulations:
            ...
            if run_state:
                ...
                # Use user-configured total_rounds; fall back to recommended rounds if absent
...
def get_simulation_profiles_realtime(simulation_id: str):
    """
    Fetch Agent Profiles in real time (for live progress viewing during generation)
...
def get_simulation_config_realtime(simulation_id: str):
    """
    Fetch simulation configuration in real time (for live progress viewing during generation)
    ...
    try:
        ...
        # If configuration exists, extract key summary statistics
        if config:
...
def get_simulation_config(simulation_id: str):
    """
    Get simulation configuration (complete config intelligently generated by LLM)
    ...
    Returns:
        ...
        - generation_reasoning: LLM reasoning explanation for the configuration
    """
    try:
        manager = SimulationManager()
...
def start_simulation():
    ...
    Request (JSON):
        {
            ...
            "max_rounds": 100,                     // optional: max simulation rounds, used to cap excessively long simulations
    ...
    About the force parameter:
        - When enabled, if the simulation is running or completed, it will be stopped first and run logs cleared
    ...
    About enable_graph_memory_update:
        - When enabled, all Agent activities (posting, commenting, liking, etc.) during simulation are updated to Zep graph in real time
        - This allows the graph to "remember" the simulation process for subsequent analysis or AI conversation
        - Requires that the simulation's associated project has a valid graph_id
    ...
    try:
        ...
        # Check whether simulation is ready
        manager = SimulationManager()
        state = manager.get_simulation(simulation_id)
        ...
        if state.status != SimulationStatus.READY:
            ...
            if is_prepared:
                ...
                # If force mode, clear run logs
                if force:
                    logger.info(f"Force mode: clearing simulation logs {simulation_id}")
...
def get_simulation_actions(simulation_id: str):
    """
    Get Agent action history for the simulation
...
def interview_agent():
    ...
    Note: This feature requires the simulation environment to be running (enters command-wait mode after completing the simulation loop)
    ...
    Request (JSON):
        {
            ...
            "platform": "twitter",             // optional, specify platform (twitter/reddit)
                                               // if not specified: interviews both platforms simultaneously (dual-platform mode)
    ...
    try:
        ...
        # Optimize prompt by adding prefix to prevent the Agent from calling tools
...
def interview_agents_batch():
    ...
    Request (JSON):
        {
            ...
            "interviews": [                    // required, list of interviews
                ...
                {
                    "agent_id": 1,
                    "prompt": "What do you think about B?"  // no platform means use default
                }
            ],
            "platform": "reddit",              // optional, default platform (overridden by each item's platform)
                                               // if not specified: interviews each Agent on both platforms simultaneously
    ...
    try:
        ...
        # Optimize each interview item's prompt by adding prefix to prevent tool calls
    ...
    Request (JSON):
        {
            ...
            "agent_id": 0,                 // optional, only fetch interview history for this Agent
...
def close_simulation_env():
    ...
    Sends a close-environment command to the simulation, causing it to exit the command-wait mode gracefully.

    Note: This differs from the /stop endpoint — /stop forcibly terminates the process,
    while this endpoint lets the simulation close the environment and exit gracefully.
