...
def generate_report():
    ...
    try:
        ...
        # Generate report_id upfront so it can be returned to the frontend immediately
        import uuid
        ...
        def run_generate():
            ...
            try:
                ...
                # Generate report (passing in the pre-generated report_id)
...
def get_report_progress(report_id: str):
    ...
    try:
        ...
        logger.error(f"Failed to fetch report progress: {str(e)}")
        return jsonify({
            "success": False,
            "error": str(e),
            "traceback": traceback.format_exc()
...
def get_report_sections(report_id: str):
    ...
    The frontend can poll this endpoint to retrieve generated section content without waiting for the full report
    ...
    try:
        ...
        logger.error(f"Failed to fetch section list: {str(e)}")
        return jsonify({
            "success": False,
            "error": str(e),
            "traceback": traceback.format_exc()
...
def get_single_section(report_id: str, section_index: int):
    ...
    try:
        ...
        logger.error(f"Failed to fetch section content: {str(e)}")
        return jsonify({
            "success": False,
            "error": str(e),
            "traceback": traceback.format_exc()
...
def check_report_status(simulation_id: str):
    """
    Check whether the simulation has a report and its status
...
def get_agent_log(report_id: str):
    ...
    Retrieves each action step in real time during report generation, including:
    - Report start, planning start/complete
    - Each section's start, tool calls, LLM responses, and completion
    - Report completion or failure

    Query parameters:
        from_line: Starting line number (optional, default 0, used for incremental fetching)
    ...
    try:
        ...
        logger.error(f"Failed to fetch Agent log: {str(e)}")
        return jsonify({
            "success": False,
            "error": str(e),
            "traceback": traceback.format_exc()
...
def get_console_log(report_id: str):
    ...
    Retrieves the console output (INFO, WARNING, etc.) generated during report creation in real time.
