...
class InsightForgeResult:
    ...
    Contains retrieval results for multiple sub-questions along with a consolidated analysis.
    """
    query: str
    ...
    def to_text(self) -> str:
        ...
        if self.key_quotes:
            ...
            for quote in self.key_quotes:
                ...
                # Filter out content containing question number prefixes (問題1-9 / "问题1-9")
                skip = False
                for d in '123456789':
                ...
                if skip:
                    continue
                # Truncate overly long content (split at period, not hard-cut)
...
class ZepToolsService:
    ...
    4. interview_agents - Deep interview (interviews simulated agents for multi-perspective views)
    ...
    def _local_search(
        ...
        Retrieves all edges/nodes and performs keyword matching locally.
    ...
    def get_all_edges(self, graph_id: str, include_temporal: bool = True) -> List[EdgeInfo]:
        """
        Get all edges in the graph (paginated retrieval, with temporal information).
    ...
    def get_node_edges(self, graph_id: str, node_uuid: str) -> List[EdgeInfo]:
        ...
        Retrieves all edges in the graph, then filters to those connected to the specified node.
        ...
        try:
            ...
            result = []
            for edge in all_edges:
                # Check if the edge involves the specified node (as source or target)
    ...
    def get_entity_summary(
        ...
        """
        Get a relation summary for the specified entity.
    ...
    def get_simulation_context(
        ...
        Performs a comprehensive search for all information related to the simulation requirements.
        ...
        # Filter entities with an actual type (not bare Entity nodes)
        entities = []
        for node in all_nodes:
    ...
    def insight_forge(
        ...
        The most powerful hybrid retrieval function; auto-decomposes the question and retrieves across multiple dimensions:
        1. Uses LLM to decompose the question into multiple sub-questions
        2. Performs semantic search for each sub-question
        3. Extracts relevant entities and retrieves their details
        4. Traces relation chains
        5. Consolidates all results and generates deep insights

        Args:
            graph_id: Graph ID
            query: User question
            simulation_requirement: Simulation requirement description
            report_context: Report context (optional; improves sub-question generation precision)
        ...
        # Fetch full details for all relevant entities (no limit, complete output)
        entity_insights = []
        node_map = {}  # Used later for relation chain construction
        ...
        # Step 4: Build all relation chains (no limit)
        relationship_chains = []
...
Requirements:
1. Each sub-question should be specific enough that related agent behavior or events can be found in the simulation world.
...
Return a JSON list of sub-questions."""
    ...
    def panorama_search(
        ...
        1. Retrieve all relevant nodes
        2. Retrieve all edges (including expired/invalid ones)
        3. Categorize current active and historical information

        This tool is suitable for scenarios where you need to understand the full picture of an event
        or track its evolution over time.
    ...
    def interview_agents(
        ...
        [Important] This feature requires the simulation environment to be running (OASIS environment active)
        ...
        # Add an optimization prefix to constrain the agent's response format
        INTERVIEW_PROMPT_PREFIX = (
            ...
            "6. Provide substantive answers; answer each question with at least 2-3 sentences\n\n"
        )
        optimized_prompt = f"{INTERVIEW_PROMPT_PREFIX}{combined_prompt}"
        
        # Step 4: Call the real interview API (no platform specified; interviews both platforms simultaneously)
        try:
            # Build batch interview list (no platform specified; interviews both platforms)
            interviews_request = []
            ...
            # Call SimulationRunner's batch interview method (no platform; both platforms)
            ...
            for i, agent_idx in enumerate(selected_indices):
                ...
                # Clean up any potential tool-call JSON wrapping
                ...
                # Strategy 2 (fallback): long text inside paired Chinese quotation marks 「」
                if not key_quotes:
    ...
    def _select_agents_for_interview(
        ...
        system_prompt = """You are a professional interview planning expert. Your task is to select the most suitable agents to interview from the simulated agent list based on the interview requirements.
...
Please select at most {max_agents} agents most suitable for interviewing and explain the selection rationale."""
...
Please generate 3-5 interview questions."""
    ...
    def _generate_interview_summary(
        ...
        system_prompt = f"""You are a professional news editor. Based on responses from multiple interviewees, generate an interview summary.
...
- {quote_instruction}
- You may use **bold** to mark key terms, but avoid other Markdown syntax"""
