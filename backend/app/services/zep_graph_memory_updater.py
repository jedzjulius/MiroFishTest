...
class AgentActivity:
    ...
    def to_episode_text(self) -> str:
        """
        Convert the activity to a text description that can be sent to Zep
        ...
        """
        # Generate a different description based on the action type
        action_descriptions = {
        ...
        # Return in "agent_name: activity description" format directly, without a simulation prefix
    ...
    def _describe_like_post(self) -> str:
        """Like a post — includes the original post text and author information"""
    ...
    def _describe_dislike_post(self) -> str:
        """Dislike a post — includes the original post text and author information"""
        post_content = self.action_args.get("post_content", "")
        post_author = self.action_args.get("post_author_name", "")
    ...
    def _describe_quote_post(self) -> str:
        """Quote post — includes the original post content, author information, and the quote comment"""
        original_content = self.action_args.get("original_content", "")
        original_author = self.action_args.get("original_author_name", "")
...
class ZepGraphMemoryUpdater:
    ...
    to the Zep graph in real time.
    Activities are grouped by platform; a batch is sent to Zep once BATCH_SIZE
    activities have accumulated for a given platform.

    All meaningful actions are sent to Zep, and action_args includes complete
    context information:
    ...
    # Batch send size (number of activities to accumulate per platform before sending)
    BATCH_SIZE = 5
    ...
    # Send interval (seconds) to avoid requests being sent too quickly
    SEND_INTERVAL = 0.5
    ...
    def __init__(self, graph_id: str, api_key: Optional[str] = None):
        ...
        # Per-platform activity buffers (each platform accumulates up to BATCH_SIZE before a batch send)
        ...
        self._skipped_count = 0     # Number of activities filtered out and skipped (DO_NOTHING)
    ...
    def add_activity(self, activity: AgentActivity):
        ...
        action_args will contain full context information (e.g. original post text, usernames, etc.).
    ...
    def _flush_remaining(self):
        ...
        # First drain remaining activities from the queue into the buffers
