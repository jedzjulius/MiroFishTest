...
class OasisProfileGenerator:
    ...
    Enhanced features:
    1. Calls the Zep graph retrieval to obtain richer context
    ...
    # Individual entity types (require generating concrete personas)
    INDIVIDUAL_ENTITY_TYPES = [
        "student", "alumni", "professor", "person", "publicfigure", 
        "expert", "faculty", "official", "journalist", "activist"
    ]
    
    # Group/organization entity types (require generating representative group personas)
    GROUP_ENTITY_TYPES = [
    ...
    def _search_zep_for_entity(self, entity: EntityNode) -> Dict[str, Any]:
        """
        Use the Zep graph hybrid search to retrieve rich information about an entity.

        Zep has no built-in hybrid search endpoint, so edges and nodes are searched
        separately and results are merged. Parallel requests are used to improve efficiency.
        ...
        def search_edges():
            """Search edges (facts/relations) - with retry logic"""
            max_retries = 3
            last_exception = None
            delay = 2.0
    ...
    def _generate_profile_with_llm(
        ...
        - Group/organization entities: generate a representative account profile
        """
    ...
    def _fix_truncated_json(self, content: str) -> str:
        """Repair JSON that was truncated due to max_tokens limits"""
        import re
    ...
    def _try_fix_json(self, content: str, entity_name: str, entity_type: str, entity_summary: str = "") -> Dict[str, Any]:
        ...
        # 1. First try to repair truncated content
        content = self._fix_truncated_json(content)
    ...
    def _get_system_prompt(self, is_individual: bool) -> str:
        ...
        base_prompt = "You are an expert in generating social media user personas. Generate detailed, realistic personas for opinion simulation that faithfully reflect existing real-world information. You must return valid JSON format; all string values must not contain unescaped newline characters."
    ...
    def _build_individual_persona_prompt(
        ...
        return f"""Generate a detailed social media user persona for this entity, faithfully reflecting existing real-world information.
...
1. bio: Social media profile description, ~200 characters.
2. persona: Detailed persona description (plain text, ~2000 characters), including:
   ...
   - Stance and views (attitude toward topics, content that might provoke or move them)
   - Unique traits (catchphrases, special experiences, personal hobbies)
   - Personal memory (important part of the persona: describe this individual's connection to the event and their existing actions/reactions in the event)
3. age: Age as a number (must be an integer)
4. gender: Gender, must be English: "male" or "female"
5. mbti: MBTI type (e.g. INTJ, ENFP)
6. country: Country name in English (e.g. "China", "United States")
...
- {get_language_instruction()} (gender field must use English male/female)
...
"""
    ...
    def _build_group_persona_prompt(
        ...
        return f"""Generate a detailed social media account profile for this organization/group entity, faithfully reflecting existing real-world information.
...
1. bio: Official account description, ~200 characters, professional and appropriate.
2. persona: Detailed account profile (plain text, ~2000 characters), including:
   - Basic organization info (official name, nature of organization, founding background, main functions)
   ...
   - Stance and attitude (official stance on core topics, how to handle controversy)
   - Special notes (profile of the group represented, operational habits)
   - Organizational memory (important part of the persona: describe this organization's connection to the event and its existing actions/responses in the event)
...
5. mbti: MBTI type to describe the account style (e.g. ISTJ for rigorous and conservative)
6. country: Country name in English (e.g. "China", "United States")
...
- {get_language_instruction()} (gender field must use English "other")
- age must be the integer 30; gender must be the string "other".
- The organization account's statements must match its identity and positioning."""
    ...
    def _generate_profile_rule_based(
        ...
        if entity_type_lower in ["student", "alumni"]:
            ...
            return {
                ...
                "mbti": "ISTJ",  # Organizational style: rigorous and conservative
                "country": "China",
                "profession": "Media",
    ...
    def generate_profiles_from_entities(
        ...
        def save_profiles_realtime():
            """Real-time save of generated profiles to file"""
            if not realtime_output_path:
                return
            
            with lock:
                # Filter to only the profiles that have been generated
    ...
    def save_profiles(
        ...
        """
        Save profiles to a file (chooses the correct format based on platform).
    ...
    def _save_twitter_csv(self, profiles: List[OasisAgentProfile], file_path: str):
        ...
        - user_char: Used internally in the LLM system prompt; determines how the agent thinks and acts
        - description: External display; visible to other users as the bio
        """
        import csv
        ...
        with open(file_path, 'w', newline='', encoding='utf-8') as f:
            ...
            # Write the headers required by OASIS
            ...
            for idx, profile in enumerate(profiles):
                ...
                row = [
                    ...
                    user_char,              # user_char: full persona (used internally by LLM)
    ...
    def _save_reddit_json(self, profiles: List[OasisAgentProfile], file_path: str):
        ...
        The user_id field is mandatory — it is the key used by OASIS agent_graph.get_agent() for matching.
        ...
        for idx, profile in enumerate(profiles):
            ...
            item = {
                "user_id": profile.user_id if profile.user_id is not None else idx,  # Critical: user_id must be included
