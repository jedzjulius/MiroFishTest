...
# China activity schedule configuration (Beijing time)
...
class AgentActivityConfig:
    ...
    # Active time slots (24-hour format, 0-23)
...
class TimeSimulationConfig:
    """Time simulation configuration (based on Chinese activity habits)"""
    # Total simulation duration (simulated hours)
    total_simulation_hours: int = 72  # Default: simulate 72 hours (3 days)

    # Time represented per round (simulated minutes) - default 60 minutes (1 hour), to accelerate time flow
    ...
    # Peak hours (evening 19-22, the most active time for Chinese users)
...
class EventConfig:
    """Event configuration"""
    # Initial events (events triggered at the start of the simulation)
    ...
    # Direction of public opinion guidance
...
class PlatformConfig:
    ...
    popularity_weight: float = 0.3  # Popularity/trending
...
class SimulationConfigGenerator:
    ...
    then automatically generates the optimal simulation parameter configuration.
...
### Basic Principles (for reference; adjust flexibly based on the specific event and participating groups):
- Infer the timezone and activity habits of the target user group based on the simulation scenario.
...
- 19-22 (evening): peak hours (activity multiplier 1.5)
- After 23:00: activity declines (activity multiplier 0.5)
- **Important**: The above values are reference examples only. Adjust based on the nature of the event and the characteristics of the participating groups.
  - Example: student groups may peak at 21-23; media may be active all day; official organizations only during work hours.
  - Example: breaking news may trigger late-night discussion; off_peak_hours may be shortened.
...
- peak_hours (int array): Peak activity hours; adjust based on the event's participant groups.
...
- reasoning (string): Brief explanation of why this configuration was chosen."""
        ...
        system_prompt = "You are a social media simulation expert. Return pure JSON format. The time configuration must match the activity habits of the target user group in the simulation scenario."
    ...
    def _parse_time_config(self, result: Dict[str, Any], num_entities: int) -> TimeSimulationConfig:
        ...
        return TimeSimulationConfig(
            ...
            off_peak_activity_multiplier=0.05,  # Almost no one active in the early morning
...
**Important**: poster_type must be chosen from the "Available Entity Types" above so that initial posts can be assigned to the right agents.
Example: Official announcements should be posted by Official/University types; news by MediaOutlet; student opinions by Student.
...
{{
    ...
    "initial_posts": [
        {{"content": "Post content", "poster_type": "EntityType (must be chosen from available types)"}},
        ...
    ],
    "reasoning": "<brief explanation>"
}}"""

        system_prompt = "You are a public opinion analysis expert. Return pure JSON format. Note: poster_type must exactly match an available entity type."
    ...
    def _assign_initial_post_agents(
        ...
        # Track the used agent index per type to avoid reusing the same agent
...
- **Activity times should match the target user group's habits** (adjust based on simulation scenario):
- **Official organizations** (University/GovernmentAgency): low activity (0.1-0.3), active during work hours (9-17), slow response (60-240 min), high influence (2.5-3.0)
...
{{
    "agent_configs": [
        ...
        system_prompt = "You are a social media behavior analysis expert. Return pure JSON. The configuration must match the activity habits of the target user group in the simulation scenario."
        ...
        for i, entity in enumerate(entities):
            ...
            # Fall back to rule-based generation if LLM did not produce a config
            if not cfg:
    ...
    def _generate_agent_config_by_rule(self, entity: EntityNode) -> Dict[str, Any]:
        """Generate a single agent configuration using rules (Chinese activity schedule)"""
        entity_type = (entity.get_entity_type() or "Unknown").lower()

        if entity_type in ["university", "governmentagency", "ngo"]:
            # Official organizations: active during work hours, low frequency, high influence
            return {
                "activity_level": 0.2,
                "posts_per_hour": 0.1,
            ...
            # Students: mainly active in the evenings, high frequency
            return {
                "activity_level": 0.8,
                "posts_per_hour": 0.6,
            ...
            # Alumni: mainly active in the evenings
            return {
                "activity_level": 0.6,
                "posts_per_hour": 0.4,
