...
Runs the simulation in the background, records every agent action, and supports real-time status monitoring.
"""
...
class SimulationRunState:
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            ...
            # Per-platform independent round counts and time
...
class SimulationRunner:
    ...
    Responsible for:
    1. Running the OASIS simulation in a background process.
    2. Parsing the run log and recording every agent action.
    3. Providing a real-time status query interface.
    ...
    def start_simulation(
        ...
        max_rounds: int = None,  # Maximum simulation rounds (optional; truncates long simulations)
        enable_graph_memory_update: bool = False,  # Whether to update agent activities to the Zep graph
        ...
        Args:
            ...
            platform: Runtime platform (twitter/reddit/parallel).
        ...
        if not os.path.exists(config_path):
            raise ValueError("Simulation configuration does not exist; please call the /prepare endpoint first")
        ...
        # If max_rounds was specified, truncate
        ...
        # If graph memory update is enabled, create an updater
        ...
        # Determine which script to run (scripts are in the backend/scripts/ directory)
        if platform == "twitter":
        ...
        try:
            # Build the run command using full paths
            # New log structure:
            ...
            # If max_rounds was specified, add it to the command-line arguments
            if max_rounds is not None and max_rounds > 0:
            ...
            # This fixes issues with third-party libraries (e.g. OASIS) that open files without specifying an encoding.
            env = os.environ.copy()
            ...
            # Set working directory to the simulation directory (database files etc. are created there).
            ...
            process = subprocess.Popen(
                ...
                start_new_session=True,  # Create a new process group so all related processes can be terminated on server shutdown
            )
    ...
    def _monitor_simulation(cls, simulation_id: str, locale: str = 'zh'):
        ...
        try:
            ...
            # After the process ends, do one final log read
            if os.path.exists(twitter_actions_log):
                        ...
                        try:
                            ...
                            if "event_type" in action_data:
                                ...
                                if event_type == "simulation_end":
                                    ...
                                    # If two platforms ran, both must be completed.
                                ...
                                # Update round information (from round_end event)
                                    ...
                                    # Overall round count is the max of both platforms
    ...
    def _check_all_platforms_completed(cls, state: SimulationRunState) -> bool:
        ...
        # Determine which platforms are enabled (by checking file existence)
    ...
    def _terminate_process(cls, process: subprocess.Popen, simulation_id: str, timeout: int = 10):
        ...
        if IS_WINDOWS:
            ...
            try:
                ...
                logger.warning(f"Process group unresponsive to SIGTERM, forcing kill: {simulation_id}")
        ...
        with open(file_path, 'r', encoding='utf-8') as f:
            for line in f:
                ...
                try:
                    ...
                    # Get platform: use the record's platform field first; fall back to default platform
    ...
    def get_all_actions(
        ...
        """
        Get the full action history across all platforms (no pagination limit).
        ...
        # Read Twitter action file (platform is automatically set to 'twitter' from the file path)
        twitter_actions_log = os.path.join(sim_dir, "twitter", "actions.jsonl")
        if not platform or platform == "twitter":
            actions.extend(cls._read_actions_from_file(
                twitter_actions_log,
                default_platform="twitter",  # Automatically populate the platform field
                platform_filter=platform,
                agent_id=agent_id, 
                round_num=round_num
            ))
        
        # Read Reddit action file (platform is automatically set to 'reddit' from the file path)
        reddit_actions_log = os.path.join(sim_dir, "reddit", "actions.jsonl")
        if not platform or platform == "reddit":
            actions.extend(cls._read_actions_from_file(
                reddit_actions_log,
                default_platform="reddit",  # Automatically populate the platform field
                platform_filter=platform,
                agent_id=agent_id,
                round_num=round_num
            ))
        
        # If per-platform files do not exist, try reading the old single-file format
        if not actions:
            actions_log = os.path.join(sim_dir, "actions.jsonl")
            actions = cls._read_actions_from_file(
                actions_log,
                default_platform=None,  # Old-format files should contain a platform field
                platform_filter=platform,
                agent_id=agent_id,
                round_num=round_num
            )
    ...
    def get_timeline(
        ...
        """
        Get the simulation timeline (summarised by round).
    ...
    def cleanup_simulation_logs(cls, simulation_id: str) -> Dict[str, Any]:
        """
        Clean up simulation run logs (used to force a restart of the simulation).
    ...
    def cleanup_all_simulations(cls):
        ...
        # Check if there is anything to clean up (avoid printing useless log entries for empty process lists)
        ...
        # First stop all graph memory updaters (stop_all logs internally)
        try:
        ...
        # Copy dict to avoid modifying it during iteration
        ...
        for simulation_id, process in processes:
            try:
                if process.poll() is None:  # Process is still running
                    ...
                    # Also update state.json, setting status to stopped
                    try:
                        sim_dir = os.path.join(cls.RUN_STATE_DIR, simulation_id)
    ...
    def register_cleanup(cls):
        ...
        Call at Flask application startup to ensure all simulation processes
        ...
        # In Flask debug mode, register cleanup only in the reloader subprocess (the process that actually runs the app).
        # WERKZEUG_RUN_MAIN=true indicates this is the reloader subprocess.
        # In non-debug mode this env var is absent, but we still need to register.
        is_reloader_process = os.environ.get('WERKZEUG_RUN_MAIN') == 'true'
        is_debug_mode = os.environ.get('FLASK_DEBUG') == '1' or os.environ.get('WERKZEUG_RUN_MAIN') is not None
        
        # In debug mode register only in the reloader subprocess; in non-debug mode always register.
        ...
        # SIGHUP exists only on Unix systems (macOS/Linux); not available on Windows
        ...
        def cleanup_handler(signum=None, frame=None):
            ...
            # Only log when there are processes to clean up
            if cls._processes or cls._graph_memory_enabled:
                logger.info(f"Received signal {signum}, starting cleanup...")
            cls.cleanup_all_simulations()
            
            # Call the original signal handler so Flask can exit normally
            if signum == signal.SIGINT and callable(original_sigint):
                ...
                # If the original handler is not callable (e.g. SIG_DFL), use default behaviour
    ...
    def check_env_alive(cls, simulation_id: str) -> bool:
        ...
        Returns:
            True if the environment is alive, False if it has been closed.
        """
        sim_dir = os.path.join(cls.RUN_STATE_DIR, simulation_id)
        if not os.path.exists(sim_dir):
            return False
    ...
    def get_env_status_detail(cls, simulation_id: str) -> Dict[str, Any]:
        """
        Get detailed status information for the simulation environment.

        Args:
            simulation_id: Simulation ID

        Returns:
            A status-detail dict containing: status, twitter_available, reddit_available, timestamp.
        """
        sim_dir = os.path.join(cls.RUN_STATE_DIR, simulation_id)
    ...
    def interview_agent(
        ...
        Args:
            ...
            platform: Target platform (optional)
                ...
                - None: In a dual-platform simulation, interview both platforms and return merged results
    ...
    def interview_agents_batch(
        ...
        Args:
            simulation_id: Simulation ID
            interviews: List of interview specs; each element contains {"agent_id": int, "prompt": str, "platform": str (optional)}
            platform: Default platform (optional; overridden by the platform field of each interview item)
                - "twitter": Default to Twitter platform only
                - "reddit": Default to Reddit platform only
                - None: In a dual-platform simulation, interview each Agent on both platforms
            timeout: Timeout in seconds
    ...
    def interview_all_agents(
        ...
        Uses the same question to interview all Agents in the simulation.

        Args:
            simulation_id: Simulation ID
            prompt: Interview question (the same question is used for all Agents)
            platform: Target platform (optional)
                - "twitter": Interview Twitter platform only
                - "reddit": Interview Reddit platform only
                - None: In a dual-platform simulation, interview each Agent on both platforms
        ...
        # Retrieve all Agent information from the config file
        config_path = os.path.join(sim_dir, "simulation_config.json")
        if not os.path.exists(config_path):
    ...
    def close_simulation_env(
        ...
        Sends a close-environment command to the simulation so it can exit the
        command-waiting loop gracefully.
    ...
    def get_interview_history(
        ...
        Args:
            ...
            platform: Platform type (reddit / twitter / None)
                - "reddit": Retrieve Reddit platform history only
                - "twitter": Retrieve Twitter platform history only
                - None: Retrieve history from both platforms
            agent_id: Specific Agent ID (optional; filters to that Agent only)
            limit: Maximum number of records to return per platform
        ...
        # When multiple platforms were queried, cap the total result count
