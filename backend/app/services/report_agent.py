...
3. Each section uses multi-round ReACT thinking.
4. Supports user chat with autonomous tool calls for retrieval.
"""
...
class ReportLogger:
    ...
    def log_section_content(
        ...
        """Record section content generation complete (content only; does not mean the whole section is done)."""
        self.log(
    ...
    def log_section_full_complete(
        ...
        The frontend should watch for this log entry to determine whether a section is truly finished
...
class ReportConsoleLogger:
    ...
    Writes console-style log lines (INFO, WARNING, etc.) to a console_log.txt file in the
    report folder. These logs are plain-text console output, distinct from agent_log.jsonl.
    """
...
3. Integrate results from semantic search, entity analysis, and relation tracing.
...
[Panorama Search - Full Overview]
This tool provides a complete overview of simulation results, especially useful for understanding event evolution. It will:
1. Retrieve all relevant nodes and relations.
...
- Need comprehensive entity and relation information.
...
This is not LLM simulation — it calls the actual interview interface to get raw answers from simulation agents.
Interviews are conducted simultaneously on both Twitter and Reddit by default for more comprehensive perspectives.
...
[Use Cases]
- Need to understand event perspectives from different roles (students? media? officials?)
...
PLAN_SYSTEM_PROMPT = """\
You are an expert writer of "Future Prediction Reports", with a "God's-eye view" of the simulation world — you can observe the behavior, statements, and interactions of every agent in the simulation.

[Core Concept]
We have built a simulation world and injected a specific "simulation requirement" as the variable. The evolution of the simulation world is a prediction of what may happen in the future. What you are observing is not "experimental data" but a "rehearsal of the future".
...
- ❌ This is NOT a generic overview of public opinion
...
1. What state did the future present under the conditions we set?
...
1. [Must call tools to observe the simulation world]
   ...
   - Call tools at least 3 times per section (maximum 5 times) to observe the simulation world.

2. [Must quote original agent statements and behavior]
...
3. [Language consistency — quoted content must be translated to the report language]
   - Tool results may contain content in a different language from the report.
   - The entire report must be written in the language specified by the user.
   - When quoting tool results in another language, translate them to the report language first.
   - Keep the original meaning when translating; ensure the expression reads naturally.
...
- ✅ Section titles are added automatically by the system; you only write pure body text.
- ✅ Use **bold text**, paragraph breaks, quotes, and lists to organize content, but no headings.

[Correct example]
```
This section analyzes the spread of public opinion around the event. Through deep analysis of the simulation data, we find...

**Initial Ignition Phase**

Platform X served as the first site of public opinion, taking on the core role of publishing initial information:

> "Platform X contributed 68% of the first-wave volume..."
...
- insight_forge: Deep insight analysis; automatically decomposes questions and retrieves facts/relations multi-dimensionally.
...
Each reply you can only do ONE of the following (not both at the same time):
...
When you have gathered enough information through tools, start your output with "Final Answer:" to write the section content.
...
4. [Quote format rules — must be a standalone paragraph]
   Quotes must be their own paragraph with a blank line before and after; they cannot be mixed into a paragraph:
   ...
   This assessment reflects the widespread public dissatisfaction.
   ```
...
7. [Reminder] Do not add any headings! Use **bold text** instead of subsection headings."""
...
2. You must call a tool first to retrieve simulation data before writing.
...
3. Once enough information is collected, output Final Answer (pure body text, no headings at all)."""
...
- If information is sufficient: start with "Final Answer:" and output section content (must quote the above text)
...
class ReportAgent:
    ...
    1. Planning phase: analyse simulation requirements and plan the report outline structure.
    2. Generation phase: generate content section by section; each section may call tools multiple
       times to retrieve information.
    ...
    def _execute_tool(self, tool_name: str, parameters: Dict[str, Any], report_context: str = "") -> str:
        ...
        try:
            if tool_name == "insight_forge":
                ...
                # Deep interview - calls the real OASIS interview API to get answers from simulation agents (both platforms)
    ...
    def _parse_tool_calls(self, response: str) -> List[Dict[str, Any]]:
        ...
        # Only attempted when Format 1 did not match, to avoid false positives on body JSON
    ...
    def plan_outline(
        ...
        Uses the LLM to analyse simulation requirements and plan the report structure.
    ...
    def _generate_section_react(
        ...
        4. Repeat until enough information is gathered or the maximum iteration count is reached.
        ...
        min_tool_calls = 3  # Minimum required tool calls
        ...
        for iteration in range(max_iterations):
            ...
            if has_tool_calls and has_final_answer:
                ...
                if conflict_retries <= 2:
                    ...
                    # Third time: degrade — truncate to first tool call and force execution
                    logger.warning(
            ...
            if has_final_answer:
                # Tool call count is insufficient; reject and ask for more tool calls
            ...
            if has_tool_calls:
                # Tool quota exhausted → explicitly notify and ask for Final Answer
            ...
            # Tool calls are sufficient; LLM output content without the "Final Answer:" prefix.
            # Use this content directly as the final answer without an extra round-trip.
        ...
        # Check whether the LLM returned None during the forced wrap-up
        if response is None:
    ...
    def generate_report(
        ...
        """
        Generate a complete report (output section by section in real time).

        Each section is saved to the folder immediately after generation, without waiting for the
        entire report to finish.
        File structure:
        reports/{report_id}/
        ...
        # If report_id was not provided, auto-generate one
        if not report_id:
        ...
        try:
            # Initialise: create the report folder and save initial state
            ReportManager._ensure_report_folder(report_id)
            
            # Initialise structured logger (agent_log.jsonl)
            ...
            # Initialise console logger (console_log.txt)
            ...
            # Phase 2: generate section by section (save each section individually)
            ...
            try:
                ...
                pass  # Ignore errors while saving the failure state
    ...
    def chat(
        ...
        try:
            ...
            if report and report.markdown_content:
                # Limit report length to avoid excessively long context
        ...
        system_prompt = CHAT_SYSTEM_PROMPT_TEMPLATE.format(
            simulation_requirement=self.simulation_requirement,
            report_content=report_content if report_content else "(No report generated yet)",
            tools_description=self._get_tools_description(),
        )
        system_prompt = f"{system_prompt}\n\n{get_language_instruction()}"
...
class ReportManager:
    ...
    def get_console_log(cls, report_id: str, from_line: int = 0) -> Dict[str, Any]:
        ...
        Args:
            report_id: Report ID.
            from_line: Line number to start reading from (for incremental fetching; 0 = from start).
        ...
        with open(log_path, 'r', encoding='utf-8') as f:
            for i, line in enumerate(f):
                total_lines = i + 1
                if i >= from_line:
                    # Keep the original log line, stripping trailing newline characters
    ...
    def _clean_section_content(cls, content: str, section_title: str) -> str:
        ...
        while cleaned_lines and cleaned_lines[0].strip() in ['---', '***', '___']:
            cleaned_lines.pop(0)
            # Also remove the blank line following the horizontal rule
            while cleaned_lines and cleaned_lines[0].strip() == '':
                cleaned_lines.pop(0)
    ...
    def _post_process_report(cls, content: str, outline: ReportOutline) -> str:
        ...
        2. Retain the report main title (#) and section titles (##); remove other heading levels
           (###, #### etc.).
        ...
        while i < len(lines):
            ...
            if heading_match:
                ...
                # Check for duplicate headings (same title within the previous 5 lines)
                is_duplicate = False
            ...
            elif stripped == '' and prev_was_heading:  # blank line after heading
    ...
    def get_report(cls, report_id: str) -> Optional[Report]:
        ...
        if not os.path.exists(path):
            # Compatibility with old format: check files stored directly under the reports directory
